...
app.factory('rule', ['qCommon', function(qCommon) {
  ...
  function calc(players, header, items, property) {
    angular.forEach(players, function(player, index) {
      ...
      if (angular.isArray(property.swedish)) {
        if (property.swedish[player.o] !== undefined) {
          player.swedish = property.swedish[player.o];
        } else {
          player.swedish = property.swedish[property.swedish.length - 1];
        }
        // swedishルールではない場合
      } else {
        player.swedish = 0;
      ...
      // pinch, chance
      player.pinch = (player.x >= property.losingPoint - player.swedish) && (player.status == 'normal');
