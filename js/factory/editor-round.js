...
app.service('round', ['qeditor', function (qeditor) {
   ...
   */
	function load(name) {
		...
		round.property = {};
		round.explain = "";
		var propertyJson = JSON.parse(fs.readFileSync(propertyJsonName, 'utf-8'));
		angular.forEach(propertyJson[0], function (value, key) {
			round.property[key] = JSON.stringify(value);
		});
		if (angular.isArray(propertyJson[1])) {
			round.explain = propertyJson[1].join("\n\n");
		}
   ...
   */
	function saveRound() {
		...
		qeditor.confirm(round.name + "を上書き保存してよろしいですか?", function (result) {
			...
			var explainJson = round.explain.replace(/\r/g, "").split("\n\n");
			// markdownに変換
			...
			var explainHtml = explainJson.map(function (exp) {
				return marked(exp);
			});
			...
			if (Object.keys(errJson).length > 0) {
				...
				fs.writeFile(propertyJsonName, JSON.stringify([propertyJson, explainJson, explainHtml]), function (err) {
					if (err) {
						qeditor.alarm(err);
					} else {
